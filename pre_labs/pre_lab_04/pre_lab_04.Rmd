...
**Task** Run the following code to load some campaign expenditure data focused on food-related expenses in Washington, D.C. **Answer** Not sure what the queston is but there are 13,101 observations and 16 variables. Looks like committee name is who is paying for the food and recipient name is the restaurant.
...
**Task** Run the following code to group and count records in the recipient_name column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** There are different spellings of W Millar that make it hard to group the same recipient together. This is true for a few other recipients too such as Walters sports bar. 
...
**Task** Run the following code to group by recipient_clean and recipient_name, then filter for cities that start with W. Name three changes the refinr function made in the first 20 results. **Answer** It changed Washington DC, Washington, DC to WASHINGTON DC. It changed WASHINGTON to Washington. It changed WASHINGTON D.C. to Washington D.C. It also changed Washington, to Washington. 
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by city_clean and city to examine cities that start with A. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** It was able to combine more of the Washington DC variations into WASHINGTONDC. 
