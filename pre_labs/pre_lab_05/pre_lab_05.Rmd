...
**Task** Load the five CSV files. Do they have the same column structure? **Answer** They all have the same columns. They are all the same kind of variables with everything but county being dbl and county is chr. 
...
#year county dem rep lib una oth total
  glimpse(county_voters_2018)
  #year county dem rep lib una oth total
glimpse(county_voters_2020)
#year county dem rep lib oth una total
glimpse(county_voters_2022)
#year county dem rep lib oth una total
glimpse(county_voters_2024)
#same as others
```
...
**Task** Load the Maryland 2020 county population data **Answer** I loaded it. I'm not sure what the question is here. 
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** 24 are there. There should be 24 I think. 
...
glimpse(maryland_population_with_voters_and_demographics)
```
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer**Prince George's County has the highest percentage of black 18+ in their population. Prince George's also had the highest percentage of Democrats with 78.29% democrats. 
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer**Howard county has the highest percentage of Asians at 19.37% and it's percentage Dems is 52.38%. For the last answer the highest percentage of black 18+ year olds and highest percentage of Democrats was the same. But Prince George's which still has the highest percentage Democrats is 5th in Asian population with 4.68%. Black people are much more loyal to the Democratic party than Asians are and this is evident in this data. 
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **Answer** It is dbl. 
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **Answer** Now it is chr.
