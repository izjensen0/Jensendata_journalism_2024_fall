...
```{r echo=FALSE}
census_api_key("c1593456bfe9d9813d469d3f50da2c0535a36217", install=TRUE)
```
...
**Task** Run the following codeblock to calculate the percentage change and put the state with the largest percentage growth below. **Answer** Nevada had the largest growth with 35% change. 
...
**Task** Run the following codeblock to find the Maryland county with the highest median household income and put the answer below. **Answer** Howard County has the heightest median household income.
...
Is the difference between Calvert and Montgomery significant? **Answer** No, not significant.

Is the difference between Howard and everyone else significant? **Answer** Yes, it is significant.
...
**Answer** Only one of the 20 zip codes with the largest totals had an estimated median income of less than 50k, 21202. 
...
**Task** Arrange md_zips_with_acs by estimate showing the highest median household income first. What questions do you have about these top zip codes? **Answer**Where are the zipcodes with the highest median incomes? What do the racial demographics of those neighborhoods look like? Are the zipcodes with the highest median income the ones that give the most to campaigns?
