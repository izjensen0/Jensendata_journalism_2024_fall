...
```{r}
create_credentials(GROQ_API_KEY = "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")
...
**Answer** I don't see any difference between the first 10 and the second 10. 
...
**Answer** It gave all the names and two ages 31 and 33. From what I can tell it printed correctly.
...
**Answer** It added job requirements to one of the person that we didn't ask for. I would also tell it to put a line between each category so it is clear which category we are looking at. I edited it and its a little better but less specific.
...
housejob_response_edit <- axolotr::ask(
  prompt = paste("Given the following text, extract information into a CSV file with the following structure with no yapping: member,party,state,district,job_title,location,hill_experience,portfolio. Each part of the structure should be on a different line.", text),
  model = "llama-3.1-8b-instant"
)

cat(housejob_response_edit)
...
**Answer** Yeah this makes sense and gives what I believe is the appropriate solution installing tidyverse.
...
**Answer** I think it makes sense. It assumes the names of the variables and the data set which might be incorrect. The code would help me understand the data. I'd have to edit it to make sure all the variables are included in the data and have the right names. 
...
**Answer**  It said "I'm sorry, but I can't view or access images. However, I can describe the different elements related to the questions I'm being asked." I don't know why there is an option to upload images if it doesn't do images.
...
**Answer** It did a decent job but just included things that can be won instead of just merchandise. So things like money and tickets which I generally wouldn't call merchandise are included. Donations is one kind of merch that was way off. I think the results make sense you get most of the information that would be needed to compare candidates merchandise. 
