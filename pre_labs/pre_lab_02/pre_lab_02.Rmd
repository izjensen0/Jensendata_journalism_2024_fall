...
**Answer** It is showing us what percentage of votes cast toward that person were made on election day rather than early or absentee. 
...
**Answer** It put the candidates in order of how much of their vote total was received on election day from most to least so the first person listed got 100% of their votes on election day. 
...
**Answer** I think it is because we can see how valuable early voting and absentee voting are to certain candidates. Since these types of voting are less traditional and have become more polarizing since the 2020 election it is good to see what types of candidates it is important to. 
...
glimpse(standardized_maryland_cities)
```

**Task** Search the Internet for tidyverse functions similar to str_to_upper. Name at least two and describe what they do.

**Answer** str_to_lower() converts to lower case. str_to_title() converts characters so that only the first letter of every word is capitalized. 
...
**Answer** The largest amount total is from the less than $100 donations. 
...
**Answer** If we don't hold it in a new variable we will not be able to use it or analyze it later. 
...
So there's 428 rows that are EITHER in Prince George's OR are senator results. OR is additive; AND is restrictive.
...
**Answer:** The date changed to year, month, day and now has the full four digit year. 
...
**Answer** The month column has the first day of each of the months that are represented in the data. I assume it uses the first day of the month to represent the entire month rather than splitting up the dates. It might keep them this way to make sure it is not grouping all Marchs for example but rather grouping by the specific march of each year. 
