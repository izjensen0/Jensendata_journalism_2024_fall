...
**Task** Export the CSV file to your pre_lab_06 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** There are no helpful column names so everything seems grouped together under different NA columns. There are also these ...1 and ...2 and so one that might be column headers but are not helpful. 
...
**Task** Get rid of rows we don't need by running the codeblock below. What problems remain? **Answer** Now there is a candidate column that includes the candidate and their party. Other than that the columns are all just X and a number which isn't helpful for determining what they are. 
...
**Task** Run the codeblock below. What changed, and what issues or problems remain? **Answer** The last answer just had a candidate column then a bunch of X columns. This one now has office, candidate, party, and then some Almira and Benzonia columns with letters after them. After that there are still a lot of X columns. Issues remain with the column titles in that other than office, candidate, and party they don't mean anything to me. 
