...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer** There are columns for precincts, congressional district, legislative (probably state) district,city council district, school district, polling ID, shape length, shape area, and general shape. Above the columns there is some information about the map including bounding box, dimensions, and geometry type.
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer**Like earlier it has information about the precincts location and different kind of districts. Shape has responses of multipolygon and multisurface with some numbers.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It's a map of  Prince George's County with longitudinal and latitudinal measures on the x and y axis. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** No output just ran the library.
...
**Task** Run the following code. Describe the output in the space below. **Answer**There are 18 columns. There is the name of the county and the FP of both county and state which I assume is the code for it. There are a lot of columns that I'm unsure what they mean such as LSAD, ClassFP, MTFCC, CSAFP, etc. There is a geometry variable at the end which I assume is important to the mapping. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** There is no output because it just put it into the md_counties item. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** It's a map of all of Maryland divided by counties. It also has the latitude and longitude. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** It's a map of just PG county and its voting precincts are layered on top of it.
...
**Task** Run the following code. Describe the output in the space below. **Answer**It is a dataset of the number fo each method of voting for each precinct. It also includes the total number of eligible voters.
...
**Task** Run the following code. Describe the output in the space below. **Answer** No output since we joined the two datasets and put them into a variable.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It is again a map of PG county but this time with the number of total eligible voters for each precinct shown by the deepness of the color blue. It is also evident that we are missing some precincts data for eligible voters.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It's a dataframe with the amount of each party votes by county. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Data shows each county and their estimated population.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The output has the data including county, the population, and the geometry variables. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** md_county_population has the counties under name with the full county name and Maryland. Voters_by_county has county listed under County.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Now there is just the county name without county or maryland, the population, and the geometry. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We joined the datasets so now there is county, population, the party votes, the total votes, and the geometry.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Just putting stuff in a variable no output.
...
**Task** Run the following code. Describe the output in the space below. **Answer** As before there is county, population, party votes, total votes, and geography but now there is also the amount of voters per 10 thousand people. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**The output is a map of maryland by county. It shows the amount of voters per 10,000 people by the depth of the color blue of the county.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It is the same as before except instead of it just using blue there are 6 colors to describe the different amount of voters per 10,000 people. 
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** The counties around dc have less voter per 10,000. This is also true albeit less so near Baltimore. The eastern side seems to have more voters per 10,000.  
...
**Task** Run the following code. Describe the output in the space below. **Answer** The code shows the unaffiliated voters per 10,000 in each county.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see, especially on the ends of the scale? **Answer** There are more unaffiliated voters in central or northern Maryland. Prince George County and Baltimore City have very few unaffiliated voters which makes sense because they are strong democratic areas. 
