...
**Task** Run the following code and describe the dataframe it outputs. **Answer** The dataframe has columns for date, title of article, url, and the text of the article.
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** The unique function takes out repeats to only give the unique values.
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer**There is one column that has every unique individual word. It is a lot of words around 677 thousand.
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** It is definitely more interesting than the stop words in the graph earlier. Health being the top word is super interesting and shows a policy focus. The other top words are more predictable like support, act, and funding. 
...
**Task** Run the following code and describe the results. **Answer** Both are lists of words without stop words from the articles that year. Health was the most important word in 2022 while it was Baltimore in 2024. 
...
**Task** Run the following code and describe the results. **Answer** The top phrase was amount included, followed by health care, Prince George's, Covid 19 among others. The inclusion of mental health and human rights is also interesting. Many common phrases are places.
...
**Task** Run the following code and describe the results. **Answer** Health care was first in this dataset followed by amount awarded, purpose funds, chesapeak bay, and human rights. Human rights being higher up than in 2022 is interesting.
...
**Task** Run the following code and describe the differences between the two results. Which one might be more useful for the press release data? **Answer**stem words sees the different tenses as the stems of the word and therefore includes driver, drive, drove, and driven whereas lemmatize words standardizes the tense and made all the words drive.  
...
**Task** Run the following code and describe the differences between the two results. Which one is more useful? **Answer** I think the lemma dataset is more helpful. The percentages of articles that include the base words is higher making it more indicative of the articles as a whole. The words are similar in both datasets but their usefulness is diluted by the different versions of the word.
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** Most seem correct if not a little general but some I think are misinterpreted. For example in the 2022 set trump is positive but I think the code is thinking of the word trump as in overcome instead of the name. Rail was marked as negative in the 2024 dataset when that is probably about the railway which is not inherently negative.
